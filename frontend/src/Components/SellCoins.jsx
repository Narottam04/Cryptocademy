...
import { interstitial, rewardVideo } from "../Utils/ads";

const SellCoins = ({ data, modal, setModal }) => {
  const { currentUser } = useAuth();
  const [coinValue, setCoinValue] = useState(1);
  const [coinUsdPrice, setCoinUsdPrice] = useState(parseFloat(data?.priceUsd));
  ...
  // Function to get the counter from local storage
  ...
  const setCounterToLocalStorage = (newCounter) => {
  ...
  const incrementCounter = () => {
    ...
    setCounterToLocalStorage(newCounter);
  };
  useEffect(() => {
  ...
  const changeCoinValue = (e) => {
    ...
    setCoinUsdPrice(data?.priceUsd * e.target.value);
  };

  const changeUsdValue = (e) => {
    setCoinUsdPrice(e.target.value);
    setCoinValue(e.target.value / data.priceUsd);
  };
  ...
  async function onPlaceOrder() {
    try {
      ...
      const portfolioUsdAmount = data.priceUsd * (availabeCoinAmt - coinValue);
    ...
    <AnimatePresence>
      {modal && (
        ...
        <motion.div
          className={`${
            !modal && "hidden"
          } flex flex-col overflow-y-auto overflow-x-hidden fixed right-0 left-0 top-[65px] z-50 justify-center items-center md:inset-0 h-modal sm:h-full`}
          id="large-modal"
            ...
            <div className="relative  rounded-lg shadow bg-gray-700">
              ...
              <div className="px-6 py-3 md:p-6">
                <p className="text-base leading-relaxed font-semibold text-gray-200">
                  1 <span className="uppercase">{data.symbol}</span> = {data.priceUsd} USD
                </p>

                <p className="text-base leading-relaxed font-semibold text-gray-200">
                  Available Balance ={" "}
                  {availableUsdCoins?.status === "success" ? availableUsdCoins?.data?.amount : 0}{" "}
                  USD
                </p>
